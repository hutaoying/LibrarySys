  ...
  //查询数据
    public Collection query(String strif){
        ...
        if(strif!="all" && strif!=null && strif!=""){
            sql="select * from t_bookType where "+strif+"";
        }else{
    ...
    public BookTypeForm queryM(BookTypeForm bookTypeForm){
        ...
        String sql="select * from t_bookType where id="+bookTypeForm.getId()+"";
...
String sql1="SELECT * FROM t_bookType WHERE typename='"+bookTypeForm.getTypeName()+"'";
...
try {
    if (rs.next()) {
        ...
        sql ="Insert into t_bookType (typename,days) values('"+bookTypeForm.getTypeName()+"',"+bookTypeForm.getDays()+")";
...
String sql="Update t_bookType set typename='"+bookTypeForm.getTypeName()+"',days="+bookTypeForm.getDays()+" where id="+bookTypeForm.getId()+"";
...
public int delete(BookTypeForm bookTypeForm){
	String sql_1="SELECT * FROM t_bookinfo WHERE typeid="+bookTypeForm.getId()+"";
	ResultSet rs=conn.executeQuery(sql_1);
	int falg=0;
	try {
		if(!rs.next()){
			String sql="Delete from t_bookType where id="+bookTypeForm.getId()+"";
			falg=conn.executeUpdate(sql);
