...
public class ReaderTypeDAO {
	...
	public Collection query(String strif) {
		...
		if (strif != "all" && strif != null && strif != "") {
			sql = "select * from t_readerType where " + strif + "";
		} else {
			sql = "select * from t_readerType";
		}
	...
	public ReaderTypeForm queryM(ReaderTypeForm readerTypeForm) {
		...
		String sql = "select * from t_readerType where id="
	...
	public int insert(ReaderTypeForm readerTypeForm) {
		String sql1 = "SELECT * FROM t_readerType WHERE name='"
		...
		try {
			if (rs.next()) {
				...
				sql = "Insert into t_readerType (name,number) values('"
						+ readerTypeForm.getName() + "',"
	...
	public int update(ReaderTypeForm readerTypeForm) {
		String sql = "Update t_readerType set name='"
	...
	public int delete(ReaderTypeForm readerTypeForm) {
		String sql_1 = "SELECT * FROM t_reader WHERE typeid="
				+ readerTypeForm.getId() + "";
