...
public class BookCaseDAO {
    ...
    public Collection query(String strif){
        ...
        if(strif!="all" && strif!=null && strif!=""){
            sql="select * from t_bookcase where "+strif+"";
        }else{
            sql="select * from t_bookcase";
        }
    ...
    public BookCaseForm queryM(BookCaseForm bookCaseForm){
        BookCaseForm bookCaseForm1=null;
        String sql="select * from t_bookcase where id="+bookCaseForm.getId()+"";
   ...
   //添加数据
    ...
    String sql1="SELECT * FROM t_bookcase WHERE name='"+bookCaseForm.getName()+"'";
    ...
    try {
        if (rs.next()) {
            ...
            sql ="Insert into t_bookcase (name) values('"+bookCaseForm.getName()+"')";
    ...
    String sql="Update t_bookcase set name='"+bookCaseForm.getName()+"' where id="+bookCaseForm.getId()+"";
    ...
    public int delete(BookCaseForm bookCaseForm){
    	String sql_1="SELECT * FROM t_bookcase WHERE bookcase="+bookCaseForm.getId()+"";
    	ResultSet rs=conn.executeQuery(sql_1);
    	int falg=0;
    	try {
    		if(!rs.next()){
    			String sql="Delete from t_bookcase where id="+bookCaseForm.getId()+"";
    			falg=conn.executeUpdate(sql);
