...
public class Borrow extends Action {
 ...
 /*********************图书借阅查询***********************/
    ...
    private ActionForward bookborrow(ActionMapping mapping, ActionForm form,
        ...
        readerForm.setNumid(request.getParameter("numid"));
        ReaderForm reader = (ReaderForm) readerDAO.queryM(readerForm);
       // System.out.println(reader);
        request.setAttribute("readerinfo", reader);
        //查询读者的借阅信息
        request.setAttribute("borrowinfo",borrowDAO.borrowinfo(request.getParameter("numid")));
        //完成借阅
        ...
        if (key != null && !key.equals("")) {
            ...
            if (bookForm!=null){
                int ret = borrowDAO.insertBorrow(reader, bookForm,
                                                 operator);
                if (ret == 1) {
                    request.setAttribute("numid", request.getParameter("numid"));
